...
template <typename T, short F, bool ROUND_BF16 = false>
kernel void kernel_rms_norm_fuse_impl(
    ...
    for (int i00 = tpitg.x; i00 < args.ne00_t; i00 += ntg.x) {
        ...
        if (F == 2) {
            const T value = (x[i00]*scale)*f0[i00];
            if (ROUND_BF16) {
                // V4.1 rounds after the weight multiply. Keep the original
                // float4 reduction and arithmetic, then round in registers.
                ...
                y[i00] = as_type<T>(bits & 0xffff0000u);
            } else {
                y[i00] = value;
            }
        }
...
template [[host_name("kernel_dsv41_rms_norm_mul_bf16_f32_4")]] kernel kernel_rms_norm_fuse_t kernel_rms_norm_fuse_impl<float4, 2, true>;
