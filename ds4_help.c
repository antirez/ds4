...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    opt(fp, c, "--ssd-streaming", "GPU graph backend: opt in to SSD-backed model streaming instead of full residency.");
    opt(fp, c, "--ssd-streaming-cold", "SSD streaming: skip default popularity-based expert-cache preload.");
    opt(fp, c, "--ssd-streaming-cache-experts N|NGB", "SSD streaming: routed expert cache as expert count or GiB, e.g. 32GB. Default: 80% GPU working set minus non-routed weights.");
