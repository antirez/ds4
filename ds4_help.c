...
static void print_server_api(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--prefill-quantum N", "Prefill slice when no generation is active. Default: 2048. Pipeline builds should set this to several --dist-prefill-chunk depths so the worker window stays full.");
