...
static void print_server_api(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--thinking-grace-tokens N", "When the token budget runs out while a thinking block is still open, allow up to N extra tokens so it can close. A reply cut mid-reasoning also poisons the session KV checkpoint. Default: 0 (off).");
