...
static void print_distributed(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--transport auto|usb4stream|rdma|tcp", "Tensor gate transport. Default: auto");
    opt(fp, c, "--usb4stream-device PATH", "Use one configured bidirectional /dev/tbstreamX device.");
