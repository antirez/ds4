/*
 * GLM-5.2 HF -> GGUF quantizer.
 *
 * Plain C, model-specific quantizer that emits a ds4-native GGUF for
 * GLM-5.2 (model architecture "glm_moe_dsa").  Reuses the ds4 quant
 * formats (Q8_0, Q4_K, Q2_K, IQ2_XXS) and produces a header/template-compatible
 * GGUF from GLM's FP8-block safetensors.
 *
 * Differences from deepseek4-quantize.c:
 *  - GLM weights are FP8-E4M3 with F32 per-128x128 block scales (weight_scale_inv),
 *    not DeepSeek's E8M0 scales; the dequant path accepts F32 as an alternative.
 *  - Routed experts use FP8, not FP4; each expert has gate_proj/up_proj/down_proj
 *    paired with a weight_scale_inv (no FP4 packed path).
 *  - GLM has standard MLA: separate kv_b_proj and a full-rank o_proj
 *    (new tensors attn_kv_b / attn_output); it has no hyper-connection or
 *    attn_sinks.
 *  - GLM has dense MLP layers 0..first_k_dense_replace-1 with gate_proj/up_proj/down_proj
 *    (3*12288), no router/router-gate/experts/shared-experts.
 *
 * The optional imatrix follows the legacy llama.cpp .dat format, as with the
 * DeepSeek tool.  When no external imatrix is supplied and IQ2_XXS requires one,
 * a synthetic weight-energy fallback is used.
 */
...
static const name_map layer_map[] = {
    /* GLM attention: standard MLA with separate kv_b and full-rank o_proj.
     * No hyper-connection (hc_*), no attn_sinks, no compressor. */
    ...
    /* Dense-MLP layers (0..first_k_dense_replace-1): standard SwiGLU triplet. */
    { "ffn_gate.weight",            "mlp.gate_proj.weight" },
    { "ffn_up.weight",              "mlp.up_proj.weight" },
    { "ffn_down.weight",            "mlp.down_proj.weight" },
    /* MoE layers: router (gate.weight, e_score_correction_bias),
     * routed experts (handled as expert tensors), shared expert. */
...
static bool is_attention_projection(const char *name) {
    return strstr(name, ".attn_kv.weight")  || strstr(name, ".attn_q_a.weight") ||
           strstr(name, ".attn_q_b.weight")  || strstr(name, ".attn_kv_b.weight") ||
...
static void generate_one_expert(expert_job *j, int xid) {
    /* GLM routed experts are FP8-E4M3 with a F32 weight_scale_inv (128x128 blocks).
     * Weight: model.layers.<L>.mlp.experts.<E>.{gate,up,down}_proj.weight +
     * Scale:   model.layers.<L>.mlp.experts.<E>.{gate,up,down}_proj.weight_scale_inv
     * expert_part_name() already emits "gate_proj"/"up_proj"/"down_proj". */
    char prefix[320];
    snprintf(prefix, sizeof(prefix), "model.layers.%d.mlp.experts.%d.%s",
             j->expert.layer, xid, j->wid);
    char weight_name[512];
    char scale_name[512];
    snprintf(weight_name, sizeof(weight_name), "%s.weight", prefix);
    snprintf(scale_name, sizeof(scale_name), "%s.weight_scale_inv", prefix);
    ...
    int64_t n = 0;
    float *f32 = dequant_fp8_weight(&w, &s, &n);
...
static void usage(const char *argv0) {
    ...
    printf("\nGLM-5.2 safetensors -> DS4-native GGUF quantizer in plain C.\n\n");
    printf("options:\n");
