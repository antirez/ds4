/*
 * Tencent Hy3 (hy_v3) HF -> GGUF quantizer.
 *
 * Plain C, model-specific quantizer that emits a ds4-native GGUF for
 * Tencent Hy3 / Hunyuan v3 (architecture "hy_v3").  Reuses the ds4 quant
 * formats (Q8_0, Q4_K, Q2_K, IQ2_XXS) and produces a header/template-compatible
 * GGUF from Hy3 plain BF16 safetensors.
 *
 * Hy3 weights are plain BF16 (no FP8 block scales). Routed experts are packed
 * into 3D ffn_{gate,up,down}_exps tensors. Metadata keys follow hy3_contract.md.
 *
 * Optional imatrix uses the legacy llama.cpp .dat format. When IQ2_XXS needs one
 * and none is supplied, a synthetic per-column weight-energy fallback is used.
 */
...
static float *dequant_fp8_weight(const st_value *w, const st_value *scale, int64_t *n_out) {
    if (strcmp(w->dtype, "F8_E4M3") != 0) die("bad FP8 weight dtype (expected F8_E4M3)");
    /* GLM uses F32 per-block scales (weight_scale_inv), DeepSeek uses E8M0.
     * Both are 2D [rows, cols] of 128x128 blocks; the last row/col block may
     * be partial (e.g. kv_a_proj_with_mqa is [576,6144], ceil to [5,48]). */
    const bool scale_is_f32 = strcmp(scale->dtype, "F32") == 0;
    const bool scale_is_e8m0 = strcmp(scale->dtype, "F8_E8M0") == 0;
    if (!scale_is_f32 && !scale_is_e8m0) die("bad FP8 scale dtype (expected F32 or F8_E8M0)");
    ...
    /* Block grid uses ceil division: scale rows/cols cover partial last blocks. */
    const int64_t scale_rows = (out_dim + block_out - 1) / block_out;
    const int64_t scale_cols = (in_dim + block_in - 1) / block_in;
    if (scale->shape[0] != scale_rows || scale->shape[1] != scale_cols) die("FP8 scale shape mismatch");
    const uint8_t *scale_u8 = scale->data;
    const float *scale_f32 = (const float *)scale->data;
    float *out = xmalloc((size_t)out_dim * (size_t)in_dim * sizeof(float));
    for (int64_t ob = 0; ob < scale_rows; ob++) {
        const int64_t row_end = (ob * block_out + block_out <= out_dim) ? block_out : (out_dim - ob * block_out);
        for (int64_t ib = 0; ib < scale_cols; ib++) {
            const int64_t col_end = (ib * block_in + block_in <= in_dim) ? block_in : (in_dim - ib * block_in);
            const float s = scale_is_e8m0
                ? e8m0_to_f32(scale_u8[(size_t)ob * (size_t)scale_cols + (size_t)ib])
                : scale_f32[(size_t)ob * (size_t)scale_cols + (size_t)ib];
...
static const char *expert_part_name(expert_part p) {
    switch (p) {
        case EXP_W1: return "gate_proj";  /* SwiGLU gate */
        case EXP_W2: return "down_proj";   /* SwiGLU down */
        case EXP_W3: return "up_proj";     /* SwiGLU up */
...
static const name_map layer_map[] = {
    /* Hy3 GQA attention (q/k/v/o_proj + per-head q_norm/k_norm). */
    { "attn_norm.weight",          "input_layernorm.weight" },
    { "attn_q_proj.weight",        "self_attn.q_proj.weight" },
    { "attn_k_proj.weight",        "self_attn.k_proj.weight" },
    { "attn_v_proj.weight",        "self_attn.v_proj.weight" },
    { "attn_o_proj.weight",        "self_attn.o_proj.weight" },
    ...
    { "ffn_gate_inp.weight",        "mlp.router.gate.weight" },
    { "exp_probs_b.bias",           "mlp.expert_bias" },
    { "ffn_gate_shexp.weight",      "mlp.shared_mlp.gate_proj.weight" },
    { "ffn_up_shexp.weight",        "mlp.shared_mlp.up_proj.weight" },
    { "ffn_down_shexp.weight",      "mlp.shared_mlp.down_proj.weight" },
};
...
static char *hf_name_for_regular(const char *gguf_name) {
    ...
    for (size_t i = 0; i < sizeof(layer_map) / sizeof(layer_map[0]); i++) {
        if (strcmp(rest, layer_map[i].gguf) == 0) {
            char buf[512];
            snprintf(buf, sizeof(buf), "model.layers.%d.%s", layer, layer_map[i].hf);
            return xstrdup(buf);
        }
    }
    fprintf(stderr, "error: cannot map GGUF tensor to HF tensor: %s\n", gguf_name);
...
static bool is_attention_projection(const char *name) {
    return strstr(name, ".attn_q_proj.weight") || strstr(name, ".attn_k_proj.weight") ||
           strstr(name, ".attn_v_proj.weight") || strstr(name, ".attn_o_proj.weight");
}

static bool is_attention_tensor(const char *name) {
    return strstr(name, ".attn") || strstr(name, "attn_");
}
...
static ds4q_type policy_type(const quant_policy *p, const char *name, const tensor_meta *tmpl) {
    ...
    if (str_ends(name, "ffn_gate_inp.weight")) return tmpl->type;
...
static void bb_push(byte_buf *b, uint8_t c) {
    b->data = xrealloc(b->data, b->size + 1);
    b->data[b->size++] = c;
}
...
static void generate_one_expert(expert_job *j, int xid) {
    /* Hy3 routed experts: plain BF16 gate_proj / up_proj / down_proj per expert. */
    char weight_name[512];
    snprintf(weight_name, sizeof(weight_name), "model.layers.%d.mlp.experts.%d.%s.weight",
...
static uint64_t hy3_metadata_kv_count(void) {
    return 24;
}

static size_t hy3_metadata_kv_size(void) {
    size_t n = 0;
    n += gguf_string_size("general.architecture") + 4 + gguf_string_size("hy_v3");
    n += gguf_string_size("general.name") + 4 + gguf_string_size("Tencent-Hy3");
    n += gguf_string_size("deepseek4.block_count") + 4 + 4;
    n += gguf_string_size("deepseek4.embedding_length") + 4 + 4;
    n += gguf_string_size("deepseek4.vocab_size") + 4 + 4;
    n += gguf_string_size("deepseek4.attention.head_count") + 4 + 4;
    n += gguf_string_size("deepseek4.attention.head_count_kv") + 4 + 4;
    n += gguf_string_size("deepseek4.attention.key_length") + 4 + 4;
    n += gguf_string_size("deepseek4.attention.value_length") + 4 + 4;
    n += gguf_string_size("deepseek4.attention.layer_norm_rms_epsilon") + 4 + 4;
    n += gguf_string_size("deepseek4.rope.freq_base") + 4 + 4;
    n += gguf_string_size("deepseek4.attention.sliding_window") + 4 + 4;
    n += gguf_string_size("deepseek4.expert_count") + 4 + 4;
    n += gguf_string_size("deepseek4.expert_used_count") + 4 + 4;
    n += gguf_string_size("deepseek4.expert_shared_count") + 4 + 4;
    n += gguf_string_size("hy.first_k_dense_replace") + 4 + 4;
    n += gguf_string_size("hy.intermediate_size") + 4 + 4;
    n += gguf_string_size("hy.expert_count") + 4 + 4;
    n += gguf_string_size("hy.expert_used_count") + 4 + 4;
    n += gguf_string_size("hy.expert_shared_count") + 4 + 4;
    n += gguf_string_size("hy.moe_intermediate_size") + 4 + 4;
    n += gguf_string_size("hy.routed_scaling_factor") + 4 + 4;
    n += gguf_string_size("hy.qk_norm") + 4 + 4;
    n += gguf_string_size("hy.moe_router_enable_expert_bias") + 4 + 4;
    return n;
}

static size_t g_tokenizer_extra_kv_size = 0;
...
static output_context build_output_context(const gguf_file *tmpl, const quant_policy *policy, const imatrix_store *im) {
    ...
    size_t kv_len = tmpl->builtin_hy3 ? hy3_metadata_kv_size() : tmpl->kv_raw_len;
    uint64_t kv_cnt = tmpl->builtin_hy3 ? hy3_metadata_kv_count() : tmpl->n_kv;
    out.meta_size = 4 + 4 + 8 + 8 + kv_len + g_tokenizer_extra_kv_size + extra_imatrix_kv_size(im) + tensor_info;
    (void)kv_cnt;
...
/* ===== Hy3 built-in GGUF metadata + tensor catalog (hy3_contract.md) ===== */

#define HY3_N_LAYERS           80
#define HY3_HIDDEN             4096
#define HY3_VOCAB              120832
#define HY3_N_HEAD             64
#define HY3_N_KV_HEAD            8
#define HY3_HEAD_DIM           128
#define HY3_DENSE_INTER        13312
#define HY3_MOE_INTER          1536
#define HY3_N_EXPERTS_DEFAULT  192
#define HY3_N_EXPERTS_USED      8
#define HY3_FIRST_K_DENSE        1
#define HY3_ROPE_THETA      11158840.0f
#define HY3_RMS_EPS            1e-5f
#define HY3_ROUTED_SCALE         2.826f

static void write_gguf_f32(FILE *fp, float v) {
    write_u32(fp, GGUF_TYPE_FLOAT32);
    if (fwrite(&v, sizeof(v), 1, fp) != 1) die("write f32 failed");
}

static void write_gguf_u32_kv(FILE *fp, const char *key, uint32_t v) {
    write_gguf_string(fp, key);
    write_u32(fp, GGUF_TYPE_UINT32);
    write_u32(fp, v);
}

/* Configurable layer count for Hy3 (default full model = 80).
 * Set by --n-layers before building the template / emitting metadata.
 * Allows producing a reduced-layer GGUF for testing from a subset of HF shards. */
static int g_hy3_active_layers = HY3_N_LAYERS;
...
static int hex4_value(const char *s) {
    int v = 0;
    for (int i = 0; i < 4; i++) {
...
static void utf8_append(byte_buf *b, uint32_t cp) {
    if (cp <= 0x7Fu) {
        bb_push(b, (uint8_t)cp);
    } else if (cp <= 0x7FFu) {
        bb_push(b, (uint8_t)(0xC0u | (cp >> 6)));
        bb_push(b, (uint8_t)(0x80u | (cp & 0x3Fu)));
    } else if (cp <= 0xFFFFu) {
        bb_push(b, (uint8_t)(0xE0u | (cp >> 12)));
        bb_push(b, (uint8_t)(0x80u | ((cp >> 6) & 0x3Fu)));
        bb_push(b, (uint8_t)(0x80u | (cp & 0x3Fu)));
    } else {
        bb_push(b, (uint8_t)(0xF0u | (cp >> 18)));
        bb_push(b, (uint8_t)(0x80u | ((cp >> 12) & 0x3Fu)));
        bb_push(b, (uint8_t)(0x80u | ((cp >> 6) & 0x3Fu)));
        bb_push(b, (uint8_t)(0x80u | (cp & 0x3Fu)));
    }
}

static char *json_unescape_string_tok(const json_doc *d, int tok) {
    const json_tok *t = &d->v[tok];
    if (t->type != JT_STRING) die("expected JSON string");
    byte_buf b = {0};
    for (int i = t->start; i < t->end; i++) {
        unsigned char c = (unsigned char)d->js[i];
        if (c != '\\') {
            bb_push(&b, c);
            continue;
        }
        if (++i >= t->end) die("bad JSON escape");
        c = (unsigned char)d->js[i];
        switch (c) {
            case '"': bb_push(&b, '"'); break;
            case '\\': bb_push(&b, '\\'); break;
            case '/': bb_push(&b, '/'); break;
            case 'b': bb_push(&b, '\b'); break;
            case 'f': bb_push(&b, '\f'); break;
            case 'n': bb_push(&b, '\n'); break;
            case 'r': bb_push(&b, '\r'); break;
            case 't': bb_push(&b, '\t'); break;
            case 'u': {
                if (i + 4 >= t->end) die("bad JSON unicode escape");
                int cp = hex4_value(d->js + i + 1);
                if (cp < 0) die("bad JSON unicode escape");
                i += 4;
                if (cp >= 0xD800 && cp <= 0xDBFF &&
                    i + 6 < t->end && d->js[i + 1] == '\\' && d->js[i + 2] == 'u') {
                    int lo = hex4_value(d->js + i + 3);
                    if (lo >= 0xDC00 && lo <= 0xDFFF) {
                        cp = 0x10000 + ((cp - 0xD800) << 10) + (lo - 0xDC00);
                        i += 6;
                    }
                }
                utf8_append(&b, (uint32_t)cp);
                break;
            }
            default:
...
static char *json_merge_pair_string(const json_doc *d, int arr) {
    int parts[2] = { -1, -1 };
    int n = 0;
    for (int i = arr + 1; i < d->len && json_is_descendant(d, i, arr); i++) {
        if (d->v[i].parent == arr) {
            if (n >= 2 || d->v[i].type != JT_STRING) die("bad tokenizer merge pair");
            parts[n++] = i;
        }
    }
    if (n != 2) die("bad tokenizer merge pair");
    char *a = json_unescape_string_tok(d, parts[0]);
    char *b = json_unescape_string_tok(d, parts[1]);
    size_t na = strlen(a), nb = strlen(b);
    char *out = xmalloc(na + 1 + nb + 1);
    memcpy(out, a, na);
    out[na] = ' ';
...
static tokenizer_meta tokenizer_load_json(const char *path) {
    tokenizer_meta tok = {0};
    size_t len = 0;
    char *text = read_file(path, &len);
    json_doc d = json_parse_text(text, len);
    int model = json_obj_get(&d, 0, "model");
    int vocab = model >= 0 ? json_obj_get(&d, model, "vocab") : -1;
    int merges = model >= 0 ? json_obj_get(&d, model, "merges") : -1;
    if (vocab < 0 || d.v[vocab].type != JT_OBJECT) die("tokenizer.json has no model.vocab object");
    if (merges < 0 || d.v[merges].type != JT_ARRAY) die("tokenizer.json has no model.merges array");

    tok.n_tokens = HY3_VOCAB;
    tok.tokens = xcalloc((size_t)tok.n_tokens, sizeof(tok.tokens[0]));
    for (int i = vocab + 1; i < d.len && d.v[i].parent == vocab;) {
        int k = i;
        int v = i + 1;
        if (v >= d.len || d.v[v].parent != vocab) die("bad tokenizer vocab object");
        int64_t id = json_i64(&d, v);
        if (id >= 0 && id < tok.n_tokens) {
            free(tok.tokens[id]);
            tok.tokens[id] = json_unescape_string_tok(&d, k);
        }
        i = json_skip(&d, v);
    }
    int added = json_obj_get(&d, 0, "added_tokens");
    if (added >= 0 && d.v[added].type == JT_ARRAY) {
        for (int i = added + 1; i < d.len && json_is_descendant(&d, i, added); i++) {
            if (d.v[i].parent != added || d.v[i].type != JT_OBJECT) continue;
            int id_tok = json_obj_get(&d, i, "id");
            int content_tok = json_obj_get(&d, i, "content");
            if (id_tok < 0 || content_tok < 0) continue;
            int64_t id = json_i64(&d, id_tok);
            if (id >= 0 && id < tok.n_tokens) {
                free(tok.tokens[id]);
                tok.tokens[id] = json_unescape_string_tok(&d, content_tok);
            }
        }
    }
    for (int i = 0; i < tok.n_tokens; i++) {
        if (!tok.tokens[i]) tok.tokens[i] = xstrdup("");
    }

    tok.n_merges = d.v[merges].size;
    tok.merges = xcalloc((size_t)tok.n_merges, sizeof(tok.merges[0]));
    int mi = 0;
    for (int i = merges + 1; i < d.len && json_is_descendant(&d, i, merges); i++) {
        if (d.v[i].parent != merges) continue;
        if (mi >= tok.n_merges) die("bad tokenizer merge count");
        if (d.v[i].type == JT_STRING) tok.merges[mi++] = json_unescape_string_tok(&d, i);
        else if (d.v[i].type == JT_ARRAY) tok.merges[mi++] = json_merge_pair_string(&d, i);
        else die("bad tokenizer merge entry");
    }
    tok.n_merges = mi;
    json_free(&d);
    free(text);
    fprintf(stderr, "loaded tokenizer: %d tokens, %d merges from %s\n", tok.n_tokens, tok.n_merges, path);
    return tok;
}

static void tokenizer_free(tokenizer_meta *tok) {
    for (int i = 0; i < tok->n_tokens; i++) free(tok->tokens[i]);
...
static uint64_t tokenizer_kv_count(const tokenizer_meta *tok) {
    return tok && tok->tokens ? 2 : 0;
}

static size_t tokenizer_string_array_kv_size(const char *key, char **items, int n) {
    size_t sz = gguf_string_size(key) + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint64_t);
    for (int i = 0; i < n; i++) sz += gguf_string_size(items[i]);
    return sz;
}

static size_t tokenizer_kv_size(const tokenizer_meta *tok) {
    if (!tok || !tok->tokens) return 0;
    return tokenizer_string_array_kv_size("tokenizer.ggml.tokens", tok->tokens, tok->n_tokens) +
           tokenizer_string_array_kv_size("tokenizer.ggml.merges", tok->merges, tok->n_merges);
}

static void write_tokenizer_string_array_kv(FILE *fp, const char *key, char **items, int n) {
    write_gguf_string(fp, key);
    write_u32(fp, GGUF_TYPE_ARRAY);
    write_u32(fp, GGUF_TYPE_STRING);
    write_u64(fp, (uint64_t)n);
    for (int i = 0; i < n; i++) write_gguf_string(fp, items[i]);
}

static void write_tokenizer_kvs(FILE *fp, const tokenizer_meta *tok) {
    if (!tok || !tok->tokens) return;
    write_tokenizer_string_array_kv(fp, "tokenizer.ggml.tokens", tok->tokens, tok->n_tokens);
    write_tokenizer_string_array_kv(fp, "tokenizer.ggml.merges", tok->merges, tok->n_merges);
}

static void write_hy3_metadata_kvs(FILE *fp) {
    write_gguf_string(fp, "general.architecture");
    write_u32(fp, GGUF_TYPE_STRING);
    write_gguf_string(fp, "hy_v3");
    write_gguf_string(fp, "general.name");
    write_u32(fp, GGUF_TYPE_STRING);
    write_gguf_string(fp, "Tencent-Hy3");
    write_gguf_u32_kv(fp, "deepseek4.block_count", (uint32_t)g_hy3_active_layers);
    write_gguf_u32_kv(fp, "deepseek4.embedding_length", HY3_HIDDEN);
    write_gguf_u32_kv(fp, "deepseek4.vocab_size", HY3_VOCAB);
    write_gguf_u32_kv(fp, "deepseek4.attention.head_count", HY3_N_HEAD);
    write_gguf_u32_kv(fp, "deepseek4.attention.head_count_kv", HY3_N_KV_HEAD);
    write_gguf_u32_kv(fp, "deepseek4.attention.key_length", HY3_HEAD_DIM);
    write_gguf_u32_kv(fp, "deepseek4.attention.value_length", HY3_HEAD_DIM);
    write_gguf_string(fp, "deepseek4.attention.layer_norm_rms_epsilon");
    write_gguf_f32(fp, HY3_RMS_EPS);
    write_gguf_string(fp, "deepseek4.rope.freq_base");
    write_gguf_f32(fp, HY3_ROPE_THETA);
    write_gguf_u32_kv(fp, "deepseek4.attention.sliding_window", 0);
    write_gguf_u32_kv(fp, "deepseek4.expert_count", HY3_N_EXPERTS_DEFAULT);
    write_gguf_u32_kv(fp, "deepseek4.expert_used_count", HY3_N_EXPERTS_USED);
    write_gguf_u32_kv(fp, "deepseek4.expert_shared_count", 1);
    write_gguf_u32_kv(fp, "hy.first_k_dense_replace", HY3_FIRST_K_DENSE);
    write_gguf_u32_kv(fp, "hy.intermediate_size", HY3_DENSE_INTER);
    write_gguf_u32_kv(fp, "hy.expert_count", HY3_N_EXPERTS_DEFAULT);
    write_gguf_u32_kv(fp, "hy.expert_used_count", HY3_N_EXPERTS_USED);
    write_gguf_u32_kv(fp, "hy.expert_shared_count", 1);
    write_gguf_u32_kv(fp, "hy.moe_intermediate_size", HY3_MOE_INTER);
    write_gguf_string(fp, "hy.routed_scaling_factor");
    write_gguf_f32(fp, HY3_ROUTED_SCALE);
    write_gguf_u32_kv(fp, "hy.qk_norm", 1);
    write_gguf_u32_kv(fp, "hy.moe_router_enable_expert_bias", 1);
}

static void hy3_add_tensor_meta(tensor_meta **list, int *n, int *cap,
                               char *name, ds4q_type type, int n_dims, const int64_t *ne) {
    if (*n >= *cap) {
        *cap = *cap ? *cap * 2 : 64;
        *list = xrealloc(*list, (size_t)*cap * sizeof(**list));
    }
    tensor_meta *t = &(*list)[(*n)++];
    t->name = name;
    t->type = type;
    t->n_dims = n_dims;
...
static gguf_file hy3_builtin_template(void) {
    gguf_file g = {0};
    g.path = xstrdup("(builtin hy_v3)");
    g.version = 3;
    g.alignment = DS4_GGUF_DEFAULT_ALIGNMENT;
    g.n_experts = HY3_N_EXPERTS_DEFAULT;
    g.builtin_hy3 = true;
    int cap = 0;
    int n = 0;
    tensor_meta *tm = NULL;
    const int64_t q_out = (int64_t)HY3_N_HEAD * HY3_HEAD_DIM;
    const int64_t kv_out = (int64_t)HY3_N_KV_HEAD * HY3_HEAD_DIM;
    int64_t ne2[2];
    char buf[128];
    snprintf(buf, sizeof(buf), "token_embd.weight");
    ne2[0] = HY3_HIDDEN; ne2[1] = HY3_VOCAB;
    hy3_add_tensor_meta(&tm, &n, &cap, xstrdup(buf), DS4Q_TYPE_F16, 2, ne2);
    snprintf(buf, sizeof(buf), "output_norm.weight");
    ne2[0] = HY3_HIDDEN; ne2[1] = 1;
    hy3_add_tensor_meta(&tm, &n, &cap, xstrdup(buf), DS4Q_TYPE_F32, 1, ne2);
    snprintf(buf, sizeof(buf), "output.weight");
    ne2[0] = HY3_HIDDEN; ne2[1] = HY3_VOCAB;
    hy3_add_tensor_meta(&tm, &n, &cap, xstrdup(buf), DS4Q_TYPE_Q8_0, 2, ne2);
    for (int il = 0; il < g_hy3_active_layers; il++) {
        char prefix[32];
        snprintf(prefix, sizeof(prefix), "blk.%d.", il);
        #define ADD1(suffix, typ) do { \
            char *nm = xmalloc(strlen(prefix) + strlen(suffix) + 1); \
            strcpy(nm, prefix); strcat(nm, suffix); \
            int64_t _ne[DS4Q_MAX_DIMS]; int _nd; \
            _nd = 1; _ne[0] = HY3_HIDDEN; \
            hy3_add_tensor_meta(&tm, &n, &cap, nm, typ, _nd, _ne); \
        } while (0)
        #define ADD2(suffix, r, c, typ) do { \
            char *nm = xmalloc(strlen(prefix) + strlen(suffix) + 1); \
            strcpy(nm, prefix); strcat(nm, suffix); \
            int64_t _ne[2] = { (c), (r) }; \
            hy3_add_tensor_meta(&tm, &n, &cap, nm, typ, 2, _ne); \
        } while (0)
        #define ADD3(suffix, d0, d1, d2, typ) do { \
            char *nm = xmalloc(strlen(prefix) + strlen(suffix) + 1); \
            strcpy(nm, prefix); strcat(nm, suffix); \
            int64_t _ne[3] = { (d2), (d1), (d0) }; \
            hy3_add_tensor_meta(&tm, &n, &cap, nm, typ, 3, _ne); \
        } while (0)
        ADD1("attn_norm.weight", DS4Q_TYPE_F32);
        ADD2("attn_q_proj.weight", q_out, HY3_HIDDEN, DS4Q_TYPE_Q8_0);
        ADD2("attn_k_proj.weight", kv_out, HY3_HIDDEN, DS4Q_TYPE_Q8_0);
        ADD2("attn_v_proj.weight", kv_out, HY3_HIDDEN, DS4Q_TYPE_Q8_0);
        ADD2("attn_o_proj.weight", HY3_HIDDEN, q_out, DS4Q_TYPE_Q8_0);
        { char *nm = xmalloc(strlen(prefix) + 20); strcpy(nm, prefix); strcat(nm, "attn_q_norm.weight");
          int64_t _ne[1] = { HY3_HEAD_DIM }; hy3_add_tensor_meta(&tm, &n, &cap, nm, DS4Q_TYPE_F32, 1, _ne); }
        { char *nm = xmalloc(strlen(prefix) + 20); strcpy(nm, prefix); strcat(nm, "attn_k_norm.weight");
          int64_t _ne[1] = { HY3_HEAD_DIM }; hy3_add_tensor_meta(&tm, &n, &cap, nm, DS4Q_TYPE_F32, 1, _ne); }
        ADD1("ffn_norm.weight", DS4Q_TYPE_F32);
        if (il < HY3_FIRST_K_DENSE) {
            ADD2("ffn_gate.weight", HY3_DENSE_INTER, HY3_HIDDEN, DS4Q_TYPE_Q8_0);
            ADD2("ffn_up.weight", HY3_DENSE_INTER, HY3_HIDDEN, DS4Q_TYPE_Q8_0);
            ADD2("ffn_down.weight", HY3_HIDDEN, HY3_DENSE_INTER, DS4Q_TYPE_Q8_0);
        } else {
            ADD2("ffn_gate_inp.weight", HY3_N_EXPERTS_DEFAULT, HY3_HIDDEN, DS4Q_TYPE_F16);
            { char *nm = xmalloc(strlen(prefix) + 20); strcpy(nm, prefix); strcat(nm, "exp_probs_b.bias");
              int64_t _ne[1] = { HY3_N_EXPERTS_DEFAULT }; hy3_add_tensor_meta(&tm, &n, &cap, nm, DS4Q_TYPE_F32, 1, _ne); }
            ADD3("ffn_gate_exps.weight", HY3_N_EXPERTS_DEFAULT, HY3_MOE_INTER, HY3_HIDDEN, DS4Q_TYPE_IQ2_XXS);
            ADD3("ffn_up_exps.weight", HY3_N_EXPERTS_DEFAULT, HY3_MOE_INTER, HY3_HIDDEN, DS4Q_TYPE_IQ2_XXS);
            ADD3("ffn_down_exps.weight", HY3_N_EXPERTS_DEFAULT, HY3_HIDDEN, HY3_MOE_INTER, DS4Q_TYPE_Q2_K);
            ADD2("ffn_gate_shexp.weight", HY3_MOE_INTER, HY3_HIDDEN, DS4Q_TYPE_Q8_0);
            ADD2("ffn_up_shexp.weight", HY3_MOE_INTER, HY3_HIDDEN, DS4Q_TYPE_Q8_0);
            ADD2("ffn_down_shexp.weight", HY3_HIDDEN, HY3_MOE_INTER, DS4Q_TYPE_Q8_0);
        }
        #undef ADD1
        #undef ADD2
        #undef ADD3
    }
    g.n_tensors = (uint64_t)n;
    g.tensors = tm;
    char **keys = xcalloc((size_t)n, sizeof(char *));
    for (int i = 0; i < n; i++) keys[i] = tm[i].name;
    hmap_build(&g.tensor_map, keys, n);
    free(keys);
    return g;
}
...
static void write_full_gguf(st_db *db, const gguf_file *tmpl, const output_context *out_ctx,
                            ...
                            const imatrix_store *imatrix, const tokenizer_meta *tokenizer) {
    ...
    if (tmpl->builtin_hy3) {
        write_u64(fp, hy3_metadata_kv_count() + tokenizer_kv_count(tokenizer) + out_ctx->n_kv_extra);
        write_hy3_metadata_kvs(fp);
        write_tokenizer_kvs(fp, tokenizer);
    } else {
...
static void usage(const char *argv0) {
    ...
    printf("\nTencent Hy3 (hy_v3) BF16 safetensors -> DS4-native GGUF quantizer.\n\n");
    printf("options:\n");
    printf("  --hf DIR               Hugging Face Hy3 directory (model.safetensors.index.json or shards)\n");
    printf("  --template FILE        optional GGUF for tensor order/shapes; default built-in hy_v3 catalog\n");
    printf("  --out FILE             output GGUF path\n");
    printf("  --compare-gguf FILE    reference GGUF for --compare-tensor\n");
    ...
    printf("  --tokenizer FILE       tokenizer.json to embed as tokenizer.ggml tokens/merges\n");
    printf("  --imatrix-strict       fail if a quantized tensor has no matching imatrix vector\n");
    printf("  --experts TYPE         routed gate/up/down expert tensors (default iq2_xxs)\n");
    ...
    printf("  --attention-proj TYPE  attn_q/k/v/o_proj type (default q8_0)\n");
    printf("  --attention TYPE       other 2D attention tensors\n");
    printf("  --shared TYPE          shared expert type (default q8_0)\n");
    printf("  --embedding TYPE       token_embd (default f16)\n");
    printf("  --output TYPE          output.weight (default q8_0)\n");
    printf("  --dense TYPE           dense FFN + remaining 2D weights (default q8_0)\n");
    printf("  --tensor-type PFX=TYPE exact tensor-name or prefix override; may repeat\n");
    printf("  --n-experts N          routed expert count (default 192)\n");
    printf("  --n-layers N           layer count (default %d; fewer N for partial-shard test GGUF)\n", HY3_N_LAYERS);
...
static params parse_args(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "-h") == 0 || strcmp(arg, "--help") == 0) {
            ...
            p.tokenizer_json = need_value(argc, argv, &i, arg);
        ...
        } else if (strcmp(arg, "--n-layers") == 0) {
            p.n_layers = atoi(need_value(argc, argv, &i, arg));
...
int main(int argc, char **argv) {
    ...
    /* Layer count is metadata-driven; default to full 80-layer Hy3 unless --n-layers overrides. */
    if (p.n_layers != 0 && (p.n_layers < 1 || p.n_layers > HY3_N_LAYERS)) {
        fprintf(stderr, "error: --n-layers out of range [1, %d]: %d\n", HY3_N_LAYERS, p.n_layers);
        exit(1);
    }
    g_hy3_active_layers = (p.n_layers > 0) ? p.n_layers : HY3_N_LAYERS;
    tokenizer_meta tokenizer = {0};
    if (!p.tokenizer_json) {
        char tok_path[PATH_MAX];
        snprintf(tok_path, sizeof(tok_path), "%s/tokenizer.json", p.hf_dir);
        if (file_exists(tok_path)) p.tokenizer_json = xstrdup(tok_path);
    }
    if (p.tokenizer_json) {
        tokenizer = tokenizer_load_json(p.tokenizer_json);
        g_tokenizer_extra_kv_size = tokenizer_kv_size(&tokenizer);
    } else {
        fprintf(stderr, "warning: tokenizer.json not found; generated GGUF will not support text prompts\n");
        g_tokenizer_extra_kv_size = 0;
    }
    ...
    if (p.template_gguf) {
        ...
        tmpl = hy3_builtin_template();
        fprintf(stderr, "using built-in hy_v3 tensor catalog and metadata\n");
    }
    if (p.policy.routed_w1 == DS4Q_TYPE_COUNT) p.policy.routed_w1 = p.policy.routed_w3 = DS4Q_TYPE_IQ2_XXS, p.policy.routed_w2 = DS4Q_TYPE_Q2_K;
    if (p.policy.dense == DS4Q_TYPE_COUNT) p.policy.dense = DS4Q_TYPE_Q8_0;
    if (p.policy.attention_proj == DS4Q_TYPE_COUNT) p.policy.attention_proj = DS4Q_TYPE_Q8_0;
    if (p.policy.shared == DS4Q_TYPE_COUNT) p.policy.shared = DS4Q_TYPE_Q8_0;
    if (p.policy.embedding == DS4Q_TYPE_COUNT) p.policy.embedding = DS4Q_TYPE_F16;
    if (p.policy.output == DS4Q_TYPE_COUNT) p.policy.output = DS4Q_TYPE_Q8_0;
    if (p.n_experts <= 0) {
        if (tmpl.n_experts > 0) {
            p.n_experts = tmpl.n_experts;
            fprintf(stderr, "using %d routed experts from metadata\n", p.n_experts);
        } else {
            p.n_experts = HY3_N_EXPERTS_DEFAULT;
            fprintf(stderr, "using default %d routed experts for Hy3\n", p.n_experts);
        }
    } else {
    ...
    if (p.dry_run) { tokenizer_free(&tokenizer); return 0; }
    ...
    write_full_gguf(&db, &tmpl, &out_ctx, p.out_gguf, p.n_experts, p.n_threads, &imatrix, &tokenizer);
