...
bool ds4_acp_json_string(const char **p, char **out) {
    ...
    if (**p != '"') goto fail;
    (*p)++;
    if (b.ptr && !acp_utf8_valid(b.ptr, b.len)) goto fail;
    *out = acp_buf_take(&b);
    return true;
...
void ds4_acp_message_free(ds4_acp_message *m) {
    free(m->id_json);
    free(m->method);
    free(m->params_json);
    free(m->result_json);
...
ds4_acp_parse_result ds4_acp_parse_message(const char *json,
                                           ds4_acp_message *out,
    ...
    while (*p && *p != '}') {
        ...
        if (!strcmp(key, "jsonrpc")) {
            ...
            free(out->result_json);
            out->result_json = NULL;
            if (!ds4_acp_json_raw_value(&p, &out->result_json)) {
                free(key);
                snprintf(err, err_len, "invalid result");
                ds4_acp_message_free(out);
                return DS4_ACP_PARSE_JSON;
            }
            out->has_result = true;
            ...
            free(out->error_json);
            out->error_json = NULL;
            if (!ds4_acp_json_raw_value(&p, &out->error_json)) {
                free(key);
                snprintf(err, err_len, "invalid error");
                ds4_acp_message_free(out);
                return DS4_ACP_PARSE_JSON;
            }
            out->has_error = true;
    ...
    if (!saw_jsonrpc || !jsonrpc_ok ||
        (out->has_id && !json_id_value(out->id_json)))
    {
        snprintf(err, err_len, "invalid JSON-RPC message");
        ds4_acp_message_free(out);
        return DS4_ACP_PARSE_REQUEST;
    }
    if (out->has_method && (out->has_result || out->has_error)) {
        snprintf(err, err_len, "request cannot include result or error");
        ds4_acp_message_free(out);
        return DS4_ACP_PARSE_REQUEST;
    }
    if (!out->has_method && out->has_result && out->has_error) {
        snprintf(err, err_len, "response cannot include result and error");
        ds4_acp_message_free(out);
        return DS4_ACP_PARSE_REQUEST;
    }
    if (!out->has_method &&
        (!out->has_id || (!out->has_result && !out->has_error)))
    {
        snprintf(err, err_len, "invalid JSON-RPC response");
        ds4_acp_message_free(out);
        return DS4_ACP_PARSE_REQUEST;
    }
...
ds4_acp_parse_result ds4_acp_parse_request(const char *json,
    ...
    memset(out, 0, sizeof(*out));
    ds4_acp_message msg;
    ds4_acp_parse_result rc = ds4_acp_parse_message(json, &msg, err, err_len);
    if (rc != DS4_ACP_PARSE_OK) return rc;
    if (!msg.has_method) {
        snprintf(err, err_len, "missing method");
        ds4_acp_message_free(&msg);
        return DS4_ACP_PARSE_REQUEST;
    }
    out->has_id = msg.has_id;
    out->id_json = msg.id_json;
    out->method = msg.method;
    out->has_params = msg.has_params;
    out->params_json = msg.params_json;
    msg.id_json = NULL;
    msg.method = NULL;
