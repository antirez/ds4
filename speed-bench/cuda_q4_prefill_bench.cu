    ...
    auto validate = [&](const char *phase) {
        for (uint32_t set = 0; set < cfg.sets; set++) {
            if (!oracle_prepare() || !select_arm(which) ||
                !which.dispatch(set) || !ds4_gpu_synchronize() ||
                !oracle(set)) {
                std::fprintf(stderr,
    ...
    auto validate = [&](const char *phase) {
        for (uint32_t set = 0; set < cfg.sets; set++) {
            if (!oracle_prepare() || !select_arm(baseline) ||
                !baseline.dispatch(set) ||
                !ds4_gpu_synchronize() || !select_arm(candidate) ||
                !candidate.dispatch(set) ||
...
bool verify_mmq_prefill_dispatch(const model_fixture &model) {
    // For N > 8 the public pair API succeeds only through MMQ. Use this
    // small probe to initialize and attest the process-wide decision.
...
int main(int argc, char **argv) {
    ...
    bool ok = true;
    model_fixture model;
    if (!make_model(&model, cfg.sets, kOutputMinB)) {
        std::fprintf(stderr,
