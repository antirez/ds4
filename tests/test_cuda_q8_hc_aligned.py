...
def production():
    ...
    assert dispatcher.count("quantize_q8_0_f32_kernel<<<") == 1
    assert "quantize_q8_0_f32_kernel<<<(unsigned)blocks, 32, 0, cuda_decode_stream()>>>" in dispatcher
...
def generate(native):
    ...
    if native:
        source = (ROOT / "ds4_cuda.cu").read_text()
        kernels = extract_function(source,
...
def policy_source():
    ...
    quant_launch = "quantize_q8_0_f32_kernel<<<(unsigned)blocks, 32, 0, cuda_decode_stream()>>>("
    assert dispatch.count(quant_launch) == 1
    dispatch = dispatch.replace(quant_launch,
        "quantize_q8_0_f32_kernel_host((unsigned)blocks, 32, 0, cuda_decode_stream(), ")
...
static void quantize_q8_0_f32_kernel_host(unsigned grid,unsigned threads,unsigned shared,int stream,int8_t *q,float *scale,
        const float *,uint64_t k,uint64_t blocks) {
    ++quants;require(grid==blocks && blocks==(k+31)/32 && threads==32 && shared==0 && stream==37,"upstream quantize launch layout/stream");
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="ds4-q8-hc-aligned-") as tmp:
        ...
        if args.cuda:
            compiler = shlex.split(os.environ.get("NVCC") or "nvcc")
