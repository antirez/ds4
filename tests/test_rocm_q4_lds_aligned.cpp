...
void select_arm(bool candidate) {
    env(rollback, candidate ? nullptr : "1");
}
struct tensor {
    ds4_gpu_tensor *p;
...
void dense_case(const model &m, uint32_t n, uint32_t k = K, uint32_t rows = M,
    ...
    for (bool candidate : {false,true}) {
        select_arm(candidate); out.prepare(); reset_counts();
...
void attention_case(const model &m) {
    ...
    for (bool candidate : {false,true}) {
        select_arm(candidate); low.prepare(); out.prepare(); reset_counts();
        ...
        check(ds4_gpu_synchronize() != 0,"attention completion");
...
int main(int argc, char **argv) {
    try {
        ...
        if (bench) {
            ...
            std::puts("A=packed LDS ROWS32; B=aligned LDS ROWS32.");
