...
static Result run_cuda(Case &c, bool aligned, bool graph) {
    Result result(c);
    ...
    auto launch = [&] {
        quantize_q8_0_f32_kernel<<<c.blocks, 32, 0, stream>>>(
    ...
    if (graph) {
        ...
        for (unsigned replay = 1; replay <= 3; ++replay) {
            ...
            const Result reference = run_cuda(c, false, false);
...
#ifdef __CUDACC__
        const Result a = run_cuda(c, false, false);
        const Result b = run_cuda(c, true, false);
        compare(a, b);
        if (shape[1] == 129 || shape[1] == 4096) {
            (void)run_cuda(c, true, true);
        }
#else
