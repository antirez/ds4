...
int main(void) {
     ...
     * Exercise its transactional publication independently from lazy use. */
    {
        ...
        const uint64_t generation_before_remap =
            ds4_gpu_q4_attn_q_b_f16_cache_generation();
        CHECK(ds4_gpu_set_model_map(model, model_bytes) != 0,
              "identical model mapping reuse");
        ds4_gpu_test_q4_attn_q_b_f16_cache_report(&prewarm_report);
        CHECK(prewarm_report.entries == 2u &&
              ds4_gpu_q4_attn_q_b_f16_cache_generation() == generation_before_remap,
              "identical mapping invalidated the cache");
        CHECK(posix_memalign(&replacement_model, (size_t)page,
                             (size_t)model_bytes) == 0,
              "replacement model allocation");
        memcpy(replacement_model, model, (size_t)model_bytes);
        CHECK(ds4_gpu_set_model_map(replacement_model, model_bytes) != 0,
              "replace resident model mapping");
        ds4_gpu_test_q4_attn_q_b_f16_cache_report(&prewarm_report);
        CHECK(prewarm_report.entries == 0u && prewarm_report.bytes == 0u &&
              ds4_gpu_q4_attn_q_b_f16_cache_generation() != generation_before_remap,
              "replacement mapping retained stale sidecars");
        CHECK(ds4_gpu_set_model_map(model, model_bytes) != 0,
              "restore original model mapping");
        CHECK(ds4_gpu_prepare_q4_attn_q_b_f16_sidecars(
                  model, model_bytes, descs, 2u, 64u, 0u,
                  &prepared_bytes) == 1 && prepared_bytes == 2u * f16_cache_bytes,
              "rebuild sidecars after mapping replacement");
        fprintf(stderr, "Metal Q4 cache mapping identity lifecycle: PASS\n");
    ...
    /* SSD streaming drops resident-only sidecars and, under REQUIRE,
     * rejects the persistent cache path without rebuilding it. The independent
     * transient path is exercised below with REQUIRE cleared. */
    ...
    fprintf(stderr,
            "Metal Q4 attn_q_b F16 cache SSD exclusion/lifecycle: PASS\n");
     ...
     * production encoder boundaries and prove that queue-local scratch is
     * not overwritten before the preceding F16/F16 consumer and tail. */
    {
        ...
        CHECK(ds4_gpu_commands_active() != 0,
              "resident transient release needs an active batch");
        ds4_gpu_q4_attn_q_b_f16_cache_report resident_release;
        ds4_gpu_test_q4_attn_q_b_f16_cache_report(&resident_release);
        CHECK(resident_release.entries == 0u,
              "resident release must exercise transient-only ownership");
        CHECK(ds4_gpu_release_q4_attn_q_b_f16_sidecars() != 0 &&
              ds4_gpu_commands_active() == 0,
              "release failed to drain resident transient-only batch");
    ...
    /* Reuse the upstream queue's 64 MiB scratch while separately admitting
     * the SSD exact-source view against the session working-set reserve. */
    ds4_gpu_set_ssd_streaming(true);
    ...
    CHECK(ssd_transient_prepared_bytes == 0u,
          "transient SSD preflight should reuse the queue scratch");
    const uint64_t repeated_ssd_generation =
        ds4_gpu_q4_attn_q_b_f16_cache_generation();
    ds4_gpu_set_ssd_streaming(true);
    CHECK(ds4_gpu_q4_attn_q_b_f16_cache_generation() == repeated_ssd_generation,
          "repeated SSD mode invalidated transient admission");
    CHECK(ds4_gpu_begin_commands() != 0,
    ...
    CHECK(ssd_lifetime_mid.entries == 0u && ds4_gpu_commands_active() != 0,
          "SSD release must exercise transient-only active batch");
    CHECK(ds4_gpu_release_q4_attn_q_b_f16_sidecars() != 0 &&
          ds4_gpu_commands_active() == 0,
          "release failed to drain SSD transient-only batch");
