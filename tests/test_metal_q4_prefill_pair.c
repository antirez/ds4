...
static void set_pair_controls(bool disabled, bool required) {
...
static void test_shapes(fixture *f, bool required) {
    set_pair_controls(false, required);
...
static void test_default_scope(fixture *f) {
    ...
    for (int required = 0; required <= 1; required++) {
        ...
        for (size_t i = 0; i < sizeof(shapes)/sizeof(shapes[0]); i++) {
            ...
            if (rc != (required ? -1 : 0)) fail("unmeasured shape escaped default admission");
        }
    }
    set_pair_controls(false, false);
    ...
    for (size_t i = 0; i < sizeof(disable_flags)/sizeof(disable_flags[0]); i++) {
        if (setenv(disable_flags[i], "1", 1)) fail("disable flag");
        const int rc = ds4_gpu_matmul_q4_K_pair_tensor(
            f->candidate[0][0], f->candidate[0][1], f->model, f->model_size,
...
/* Only the measured M32 shape is admitted. Rejection must leave caller
 * buffers untouched, including complete tiles past the token cap. */
static void test_ineligible_shapes(fixture *f) {
    const uint32_t cases[][4] = {
        ...
        /* All weights/activations fit the fixture. The unmeasured shape must
         * be rejected before its 2.5 MiB RHS could exceed the 2 MiB scratch. */
...
int main(void) {
    ...
    fprintf(stderr,
            ...
            "bitwise=1 require=1 alias=1 ssd_spans=1 queues=1 "
            "unretained=%s\n",
