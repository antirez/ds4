...
def native_program(backend):
    ...
    candidate = ('ds4_q4_dequant_f16_pair32_kernel<<<unsigned((chunks/2u+255u)/256u),256,0,stream>>>(\n'
    ...
    # Keep both reference arms scalar: this fixture also exercises 2-byte
    # offsets that deliberately cannot use the aligned frozen vec16 kernel.
    previous = ('frozen_vec16_reference<<<unsigned((chunks+255u)/256u),256,0,stream>>>(\n'
                '            y.ptr<__half>(),x.ptr<BlockQ4>(),blocks);')
    assert program.count(previous) == 1, 'native fixture previous arm changed'
    program = program.replace(previous,
        'scalar_reference<<<unsigned((chunks+255u)/256u),256,0,stream>>>(\n'
        '            y.ptr<__half>(),x.ptr<BlockQ4>(),k,m);')
    ...
    return ('// Generated actual production flat scalar vs independent legacy scalar.\n' +
            ...
            .replace('v ? "pair32" : "old_vec16"', 'v ? "flat16" : "reference16"')
            .replace('scalar/old-vector/paired F16 parity', 'legacy/flat F16 parity')
