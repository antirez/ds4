...
static void gpu_grouped_case(int m, int k, int tokens, int groups, bool nonfinite) {
    ...
    const size_t q8_bytes = size_t(groups) * (padded_k / 32) * 36; // block_q8_1 ABI
    const size_t ids_offset = (q8_bytes + 15) / 16 * 16;
    const size_t scratch_bytes = ids_offset + size_t(groups)*sizeof(int32_t);
    ...
    auto enqueue = [&] {
        for (int token = 0; token < tokens; ++token) {
            const int rc = ds4_mmq_q4_K_grouped_vec(
                w.ptr<void>(), x.ptr<float>() + size_t(token)*groups*k,
                out.ptr<float>() + size_t(token)*groups*m,
                m, k, groups, stream);
            if (rc != 0) return rc;
        }
        return 0;
    ...
    const bool eligible = ds4_q4_mmvq_grouped_shape_ok(m, k, 1, groups, padded_k / 32);
    for (int repeat = 0; repeat < 3; ++repeat) {
        ...
        for (const EpilogueArm &arm : epilogue_arms) {
            ...
            const GraphCounts counts = graph_counts(graph);
            if (first_arm) reference_counts = counts;
            const size_t removed = arm.candidate && eligible ? size_t(tokens) : 0;
