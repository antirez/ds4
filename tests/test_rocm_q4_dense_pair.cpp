...
extern "C" int ds4_rocm_test_q4_prefill_wmma_requested_policy(
    int ssd_streaming, int enabled, int disabled,
    int required);
extern "C" int
...
bool run_q_b_f16_null_qhalf_case(const aligned_model &model) {
    ...
    env_snapshot transient_min("DS4_ROCM_Q4_ATTN_Q_B_TRANSIENT_F16_MIN_TOKENS");
    (void)setenv("DS4_ROCM_Q4_ATTN_Q_B_TRANSIENT_F16_MIN_TOKENS", "32", 1);
    ...
    if (!ds4_gpu_cache_model_range(model.data, model.size, model.q_b_k1024_offset, weight_bytes, "q_b transient fixture")) return false;
    ...
    // NULL q_half must produce the same F32 result as an ignored half tensor.
    int scratch_rc = 0;
    if (prepare_rc > 0) {
    ...
    default_host.resize(logical_count);
    ...
    if (scratch_rc > 0 && read_tensor(scratch_out.ptr, &scratch_host)) {
        ...
        if (default_read_ok) {
            ok = bitwise_equal(scratch_host, default_host,
    ...
    ok = release_rc != 0 && ok;
    std::fprintf(stderr,
                 "q_b F16 q_half staging: prepare=%d default=%d "
                 "scratch=%d "
                 "prepared=%.2f MiB release=%d %s\n",
                 prepare_rc, default_rc, scratch_rc,
...
bool run_prefill_wmma_requested_policy_oracle() {
    bool ok = true;
    unsigned cases = 0;
    for (int ssd : {0, 1}) for (int enabled : {-1, 0, 1})
    for (int disabled : {0, 1}) for (int required : {0, 1}) {
        const int expected = required || (!disabled && !ssd && enabled != 0);
        ok = ds4_rocm_test_q4_prefill_wmma_requested_policy(
            ssd, enabled, disabled, required) == expected && ok;
        ok = ds4_rocm_test_q4_prefill_wmma_attention_a_requested_policy(
            ssd, enabled, disabled, required) == expected && ok;
        ++cases;
    }
    const int k64_unset =
...
bool run_prefill_k1024_tile4_policy_oracle() {
    ...
    for (int ssd : {0, 1})
    for (int disabled : {0, 1}) for (int required : {0, 1}) {
        const int expected = disabled || ssd ? (required ? -1 : 0) : 1;
        ok = ds4_rocm_test_q4_prefill_k1024_tile4_policy(
            ssd, disabled, required) == expected && ok;
        ++cases;
    }
    std::fprintf(stderr,"ROCm Q4 K1024 TILE4 resident policy: %u cases %s\n",
                 cases, ok ? "PASS" : "FAIL");
    return ok;
}
...
bool run_prefill_pair_case(const aligned_model &model, uint32_t n_tokens,
    ...
    // decode calls, which retain their independent dense fallback.
    (void)unsetenv(kPrefillEnable);
    (void)unsetenv(kPrefillDisable);
    (void)setenv(kPrefillRequire, "1", 1);
...
bool run_grouped_attention_decode_case(const aligned_model &model) {
    ...
    // A non-production slice falls back; REQUIRE fails without dispatch.
    (void)unsetenv(kGroupedDecodeRequire);
    const int subset_default_rc = ds4_gpu_attention_output_low_q4_K_slice_tensor(
        subset_candidate.ptr, model.data, model.size,
        model.decode_attn_a_offset, kDecodeAttnGroupDim, kDecodeAttnRank,
        subset_group0, subset_group_cnt, subset_heads.ptr, 1);
    (void)setenv(kGroupedDecodeRequire, "1", 1);
    const int subset_required_rc = ds4_gpu_attention_output_low_q4_K_slice_tensor(
        subset_candidate.ptr, model.data, model.size,
        model.decode_attn_a_offset, kDecodeAttnGroupDim, kDecodeAttnRank,
        subset_group0, subset_group_cnt, subset_heads.ptr, 1);
    ok = subset_default_rc == 0 && subset_required_rc == -1 &&
         unchanged_after_rejected_call(subset_candidate.ptr, subset_sentinel,
             "grouped attention-A rejected slice preserves output") && ok;
    std::fprintf(stderr,"grouped attention-A production and rejection: %s\n",
                 ok ? "PASS" : "FAIL");
    return ok;
}
...
bool run_attention_output_wmma_smoke(const aligned_model &model) {
    ...
    /* REQUIRE asserts the resident A path and keeps B exact. */
    clear_controls();
    (void)setenv(kPrefillWmmaRequire, "1", 1);
    (void)setenv(kPrefillWmmaK64, "0", 1);
    ...
    const bool strict_k32_ready = batch_ready(
        strict_k32_result, 1u, 0u, "production REQUIRE A K32/B TILE8");
    const bool strict_k64_ready = batch_ready(
        strict_k64_result, 1u, 1u, "production REQUIRE A K64/B TILE8");
    ...
    if (strict_k32_result.out_read && strict_k64_result.out_read) {
        comparisons_ok = bitwise_equal(
            ...
            "production REQUIRE A-K64/B-TILE8 vs A-K32/B-TILE8") && comparisons_ok;
    } else {
        comparisons_ok = false;
    }
...
int main(int argc, char **argv) {
    ...
    if (model_ready && run_pair) {
        ok = run_prefill_pair_case(model, 9u, false, kTailK) && ok;
        ok = run_prefill_pair_case(model, 30u, true) && ok;
        ok = run_prefill_pair_case(model, 128u, false) && ok;
    }
