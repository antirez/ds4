...
static uint32_t test_k_loops() {
    ...
    for (uint32_t threads : {256u, 512u}) {
        ...
        for (uint32_t b0 = 0; b0 < blocks; b0 += BLOCKS) {
            test_tile<BLOCKS>(nt, std::min(BLOCKS, blocks - b0), threads,
...
static uint32_t test_geometry() {
    using Geometry = lds::tile_geometry<ROWS>;
    require(Geometry::threads == ROWS * 8u, "eight threads per output row");
    uint32_t cases = 0;
    for (uint32_t m : {1u, 31u, 32u, 33u, 63u, 64u, 65u, 4095u, 4096u, 4097u}) {
        std::vector<uint32_t> owners((size_t)m * 8u, 0u);
        uint32_t inactive = 0;
        const uint32_t grid = (m + ROWS - 1u) / ROWS;
        for (uint32_t block = 0; block < grid; ++block)
        for (uint32_t tid = 0; tid < Geometry::threads; ++tid) {
            const uint32_t row = Geometry::row(block, tid);
            const uint32_t lane = Geometry::lane(tid);
            require(row == block * ROWS + tid / 8u && lane == tid % 8u,
                    "row/lane mapping preserves adjacent eight-lane groups");
            if (row < m) ++owners[(size_t)row * 8u + lane];
            else ++inactive;
        }
        for (uint32_t count : owners)
            require(count == 1u, "each valid row/lane has exactly one owner");
        require(inactive == (grid * ROWS - m) * 8u,
                "only the final partial row tile has inactive owners");
        ++cases;
    }
    return cases;
}
