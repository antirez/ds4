...
ROCM_ARCH ?= gfx1151
ROCM_CFLAGS ?= -O3 $(DEBUG_FLAGS) -fno-finite-math-only -pthread -D__HIP_PLATFORM_AMD__ -Wno-unused-command-line-argument --offload-arch=$(ROCM_ARCH)
ROCM_LDLIBS ?= -lm -pthread -L$(ROCM_PATH)/lib -lhipblas
GPU_CC = $(HIPCC)
GPU_CFLAGS = $(ROCM_CFLAGS)
GPU_LDLIBS = $(ROCM_LDLIBS)
...
help:
	...
	@echo "  make rocm ROCM_ARCH=gfxNNNN Build ROCm/HIP for an AMD GPU"
	@echo "  make cpu                 Build CPU-only ./ds4, ./ds4-server, ./ds4-bench, ./ds4-eval, and ./ds4-agent"
	@echo "  make test                Build and run tests"
	@echo "  make clean               Remove build outputs"
...
rocm:
	...
	$(MAKE) ds4 ds4-server ds4-bench ds4-eval ds4-agent GPU_BACKEND=rocm ROCM_ARCH="$(ROCM_ARCH)"
