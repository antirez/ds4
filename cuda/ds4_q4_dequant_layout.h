...
enum { block_values = 256, block_bytes = 144, values_per_thread = 32,
       chunks_per_block = 16, tasks_per_block = 8, threads = 256 };
...
// A task owns matching 16-byte low/high nibble chunks in one Q4_K block.
DS4_Q4_DQ_INLINE uint64_t low_chunk(uint64_t task) {
    const uint64_t p = task & 7u;
    return (task >> 3u) * 16u + (p >> 1u) * 4u + (p & 1u);
}
