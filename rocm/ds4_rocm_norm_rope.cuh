...
// Q4 F32-output prefill epilogue; the generic/Q8 norm path is unchanged.
...
// Invoked only by the Q4 transient F16 GEMM with F32 output. Do not
...
// Native oracle hook: same validation and dispatch as production.
...
static int rocm_q4_attn_q_b_transient_f16_head_rms_rope_tail_tensor(
    ...
    if (ds4_q4_dequant::select(
        ...
        ds4_q4_dequant_f16_pair32_kernel<<<
            (uint32_t)((total_chunks / 2u + 255u) / 256u), 256>>>(
    ...
    // Preserve the F32 output consumed by the production RMS/RoPE epilogue.
    ...
    const int tail_ok = rocm_q4_qb_f32_epilogue_tensor(
