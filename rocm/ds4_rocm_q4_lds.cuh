...
// then share the result across the output rows. Keeping 32-bit group sums
...
struct tile_geometry {
    static_assert(ROWS == 32u, "Q4 TILE8 row geometry");
    static constexpr uint32_t threads = ROWS * 8u;
    DS4_Q4_LDS_INLINE static uint32_t lane(uint32_t tid) { return tid & 7u; }
    DS4_Q4_LDS_INLINE static uint32_t row(uint32_t block, uint32_t tid) {
        return block * ROWS + (tid >> 3u);
    }
};
