...
// canonical Q4_K/Q8_K block dot product used by the routed-MoE implementation.
// The optimized activation producer and reductions below are local to dense Q4.
...
// These helpers belong to dense Q4. Upstream MoE dispatch is unchanged.
__device__ static float rocm_q4_quarter_warp_sum_f32(float v, uint32_t lane8) {
...
__global__ static void rocm_q4_q8_K_quantize_kernel(cuda_block_q8_K *out, const float *x, uint32_t in_dim, uint32_t n_rows) {
    ...
    if (n_rows > 8u) {
        ...
        if (tid < 32u) {
            ...
            if (tid == 0u) {
                ...
                // This fence publishes both the winner and its scale. Keep
                // the zero block free of division, including signed zero.
                if (best.magnitude != 0.0f) {
                    maxv_s = val_part[0];
    ...
    int qv = 0;
    if (tid < CUDA_QK_K) {
        // Input and quantized scratch are distinct in the callers. Retain
        // the original F32 leaf across the fences instead of reloading it.
        qv = (int)lrintf(iscale_s * v);
        if (qv > 127) qv = 127;
        if (qv < -128) qv = -128;
        yb->qs[tid] = (int8_t)qv;
    }
 ...
 * wave32 resident prefill outside quality mode. SSD retains the exact
 * Q8_K path; DISABLE provides an authoritative rollback.
 */
enum {
...
__global__ static void rocm_matmul_q4_K_prefill_tile8_strided_kernel(
        ...
        uint64_t out_token_stride) {
    using Geometry = ds4_rocm_q4_lds::tile_geometry<32u>;
    ...
    const uint32_t row = Geometry::row(blockIdx.x, tid);
    const uint32_t tok0 = blockIdx.y * ROCM_Q4_PREFILL_TOKEN_TILE;
    const uint32_t group = blockIdx.z;
    const uint32_t nt = n_tok - tok0 < ROCM_Q4_PREFILL_TOKEN_TILE
                      ? n_tok - tok0 : ROCM_Q4_PREFILL_TOKEN_TILE;
    ...
    if (row_valid) {
        ...
        for (uint32_t p = 0u; p < ROCM_Q4_PREFILL_TOKEN_TILE; p++) {
            if (p < nt) {
                const float v = rocm_q4_quarter_warp_sum_f32(acc[p], lane);
                if (lane == 0u) {
...
static int rocm_q4_K_grouped_attn_a_resident_default_scope(
     ...
     * resident. REQUIRE only asserts this same production scope. */
 ...
 * streaming always retains the exact Q8_K path. */
static int rocm_q4_K_prefill_wmma_requested_policy(
    ...
    return !ssd_streaming && enabled != 0;
}

/* Attention output is a two-stage A -> B projection.  A retains the validated
 * standalone policy: resident execution is automatic; SSD retains Q8_K. */
static int rocm_q4_K_prefill_wmma_attention_a_requested_policy(
    ...
    return rocm_q4_K_prefill_wmma_requested_policy(
        ssd_streaming, enabled, disabled, required);
}
...
extern "C" int ds4_rocm_test_q4_prefill_wmma_requested_policy(
        ...
        int required) {
    return rocm_q4_K_prefill_wmma_requested_policy(
        ssd_streaming != 0, enabled, disabled != 0,
        required != 0);
}
...
static int rocm_q4_K_prefill_wmma_select(
    ...
    const int requested = rocm_q4_K_prefill_wmma_requested_policy(
        g_ssd_streaming_mode, enabled, disabled, required);
    ...
    fprintf(stderr,
            ...
            "(N=%llu K=%llu M=%llu disabled=%d ssd=%d "
            "quality=%d)\n",
...
/* TILE4 is automatic in resident mode. SSD retains TILE8; REQUIRE reports
 * an unavailable production path instead of extending its admission. */
 ...
 * SSD fallback and DISABLE/REQUIRE precedence. */
...
extern "C" int ds4_rocm_matmul_q4_K_tensor(
    ...
    if (prefill_wmma == ROCM_Q4_PREFILL_WMMA_USE &&
        ...
        // REQUIRE_TILE8 selects the exact arithmetic path.
    ...
    if (prefill_scope && prefill_tile8) {
        k1024_tile4 = rocm_q4_K_prefill_k1024_tile4_resolve(
            blocks, out_dim, weight_offset, weight_bytes);
        if (k1024_tile4 ==
...
extern "C" int ds4_gpu_matmul_q4_K_pair_tensor(
    ...
    if (!prefill_pair) return pre_enqueue_failure;
...
extern "C" int ds4_gpu_attention_output_low_q4_K_slice_tensor(
    ...
    rocm_q4_q8_K_quantize_kernel<<<qgrid, 256>>>(
...
extern "C" int ds4_gpu_attention_output_q4_K_batch_tensor(
    ...
    const int a_wmma_requested =
        rocm_q4_K_prefill_wmma_attention_a_requested_policy(
            g_ssd_streaming_mode, wmma_enabled,
    ...
    if (!tile8_requested && !a_wmma_requested) {
        if (tile8_required) {
            fprintf(stderr,
    ...
    int a_wmma = a_wmma_requested
        ? rocm_q4_K_prefill_wmma_select(
              n_tokens, group_dim, rank)
        : ROCM_Q4_PREFILL_WMMA_FALLBACK;
    if (a_wmma == ROCM_Q4_PREFILL_WMMA_REQUIRED_FAILURE) return -1;
    if (tile8_required && a_wmma == ROCM_Q4_PREFILL_WMMA_USE) {
        if (wmma_required) {
            fprintf(stderr,
                    DS4_GPU_LOG_PREFIX
    ...
    if (!tile8_requested &&
        (a_wmma != ROCM_Q4_PREFILL_WMMA_USE || out_b_type == 12u)) {
        return pre_enqueue_failure;
    }
