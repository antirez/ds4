...
static void *client_main(void *arg) {
    ...
    if (!strcmp(hr.method, "GET") && !strcmp(hr.path, "/")) {
        http_response(fd, s->enable_cors, 200, "text/html",
            ...
            "<style>:root{--bg:#1a1a1a;--surface:#252525;--border:#333;--text:#e0e0e0;--text-muted:#888;--accent:#4a9eff;--user-msg:#2d3748;--assistant-msg:#262626;--thinking:#2a2a30;--code-bg:#1e1e1e;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif}"
            ...
            "#input-area textarea{flex:1;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:10px 14px;color:var(--text);font-size:14px;resize:none;outline:none;max-height:200px}"
            ...
            "</style></head><body>"
            "<div id=header><h1>DwarfStar</h1><span class=status id=status-indicator>connecting...</span><button id=new-chat-btn>New Chat</button></div>"
            "<div id=chat></div>"
            "<div id=input-area><textarea id=input rows=1 placeholder='Send a message...' autofocus></textarea><button id=send-btn disabled>Send</button></div>"
            "<script>(function(){"
            "const BASE='';const chatEl=document.getElementById('chat');const inputEl=document.getElementById('input');"
            "const sendBtn=document.getElementById('send-btn');const statusEl=document.getElementById('status-indicator');"
            "const newChatBtn=document.getElementById('new-chat-btn');let messages=[];let modelId='';let sending=false;let abortCtrl=null;"
            "inputEl.addEventListener('input',function(){this.style.height='auto';this.style.height=Math.min(this.scrollHeight,200)+'px'});"
            "function setStatus(ok,t){statusEl.className='status '+(ok?'ok':'err');statusEl.textContent=t}"
            "async function init(){try{const r=await fetch(BASE+'/v1/models');if(!r.ok)throw Error(r.status);"
            "const d=await r.json();if(d.data&&d.data[0])modelId=d.data[0].id||'ds4flash.gguf';else modelId='ds4flash.gguf';"
            "setStatus(true,'connected');sendBtn.disabled=false}"
            "catch(e){setStatus(false,'offline');sendBtn.disabled=true}}init();"
            "function escapeHtml(s){const d=document.createElement('div');d.appendChild(document.createTextNode(s));return d.innerHTML}"
            "function renderMarkdown(t){let s=escapeHtml(t);"
            "s=s.replace(/```(\\w*)\\n([\\s\\S]*?)```/g,function(m,l,c){return'<pre><code>'+escapeHtml(c)+'</code></pre>'});"
            "s=s.replace(/`([^`]+)`/g,'<code>$1</code>');s=s.replace(/\\*\\*([^*]+)\\*\\*/g,'<strong>$1</strong>');"
            "s=s.replace(/\\*([^*]+)\\*/g,'<em>$1</em>');s=s.replace(/\\n/g,'<br>');return s}"
            "function renderMessage(m){const d=document.createElement('div');d.className='msg '+m.role;"
            "const n=document.createElement('div');n.className='name';n.textContent=m.role==='user'?'You':'DwarfStar';d.appendChild(n);"
            "if(m.reasoning){const rb=document.createElement('div');rb.className='thinking-block';rb.textContent=m.reasoning;d.appendChild(rb)}"
            "if(m.content){const md=document.createElement('div');md.innerHTML=renderMarkdown(m.content);d.appendChild(md)}"
            "if(m.tool_calls)for(const tc of m.tool_calls){const e=document.createElement('div');e.style.cssText='font-size:12px;color:var(--text-muted);padding:4px 0';"
            "e.textContent='🔧 '+tc.function.name+'('+tc.function.arguments+')';d.appendChild(e)}"
            "return d}"
            "function addMessage(m,isDelta){"
            "if(isDelta&&m.content===null&&!m.finish_reason)return;"
            "const last=messages[messages.length-1];"
            "if(isDelta&&last&&last.role===m.role&&!last.finish_reason){"
            "if(m.content)last.content+=m.content;if(m.finish_reason)last.finish_reason=m.finish_reason;"
            "const items=chatEl.querySelectorAll('.msg');if(items.length>0){const t=items[items.length-1];if(t.classList.contains(m.role)){const ne=renderMessage(last);t.replaceWith(ne)}}"
            "scrollToBottom();return}"
            "messages.push(m);chatEl.appendChild(renderMessage(m));scrollToBottom()}"
            "function scrollToBottom(){chatEl.scrollTop=chatEl.scrollHeight}"
            "function addTyping(){const d=document.createElement('div');d.className='typing';d.id='typing-indicator';d.textContent='DwarfStar is thinking...';chatEl.appendChild(d);scrollToBottom()}"
            "function removeTyping(){const t=document.getElementById('typing-indicator');if(t)t.remove()}"
            "function showError(t){removeTyping();const d=document.createElement('div');d.className='error-msg';d.textContent=t;chatEl.appendChild(d);scrollToBottom()}"
            "async function sendMessage(){if(sending)return;const text=inputEl.value.trim();if(!text)return;"
            "inputEl.value='';inputEl.style.height='auto';sending=true;sendBtn.disabled=true;"
            "const userMsg={role:'user',content:text};addMessage(userMsg,false);"
            "const msgs=messages.map(m=>({role:m.role==='tool'?'function':m.role,content:m.content||''}));"
            "if(msgs.length&&msgs[msgs.length-1].role==='assistant')msgs.pop();"
            "msgs.push({role:'user',content:text});addTyping();"
            "const assistantMsg={role:'assistant',content:'',reasoning:'',finish_reason:null};"
            "abortCtrl=new AbortController();let firstDelta=true;"
            "try{const body=JSON.stringify({model:modelId||'ds4flash.gguf',messages:msgs,stream:true,temperature:0.7,max_tokens:4096});"
            "const res=await fetch(BASE+'/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json'},body:body,signal:abortCtrl.signal});"
            "if(!res.ok){let et='HTTP '+res.status;try{const e=await res.json();et=e.error?.message||et}catch(_){}showError('Server error: '+et);sending=false;sendBtn.disabled=false;return}"
            ...
            "try{const parsed=JSON.parse(p);const delta=parsed.choices?.[0]?.delta||{};const finish=parsed.choices?.[0]?.finish_reason||null;"
            "if(firstDelta){removeTyping();messages.push(assistantMsg);firstDelta=false}"
            "if(delta.content)assistantMsg.content+=delta.content;assistantMsg.finish_reason=finish;"
            "const items=chatEl.querySelectorAll('.msg');const lastEl=items[items.length-1];"
            "if(lastEl&&lastEl.classList.contains('assistant')){const ne=renderMessage(assistantMsg);lastEl.replaceWith(ne)}else{chatEl.appendChild(renderMessage(assistantMsg))}"
            "scrollToBottom()}catch(e){}}}"
            "if(firstDelta)removeTyping();if(!firstDelta&&!assistantMsg.finish_reason){assistantMsg.finish_reason='stop';"
            "const items=chatEl.querySelectorAll('.msg');const lastEl=items[items.length-1];if(lastEl&&lastEl.classList.contains('assistant')){const ne=renderMessage(assistantMsg);lastEl.replaceWith(ne)}}"
            "}catch(e){if(e.name==='AbortError')removeTyping();else showError('Connection error: '+e.message)}"
            "sending=false;sendBtn.disabled=false}"
            "function newChat(){if(sending&&abortCtrl)abortCtrl.abort();sending=false;sendBtn.disabled=false;messages=[];chatEl.innerHTML='';inputEl.value='';inputEl.style.height='auto';removeTyping()}"
            "sendBtn.addEventListener('click',sendMessage);"
            "inputEl.addEventListener('keydown',function(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendMessage()}});"
            "newChatBtn.addEventListener('click',newChat)})()</script></body></html>");
        http_request_free(&hr);
        goto done;
    }
