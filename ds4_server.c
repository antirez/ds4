...
static void remember_transient_checkpoint(server *s, const job *j, const char *ctx,
                                          uint64_t trace_id, const char *content,
                                          const char *reasoning,
                                          const tool_calls *calls) {
    char *visible = NULL;
    ...
    thinking_live_remember(s, stripped);
    server_log(DS4_LOG_KVCACHE,
               "ds4-server: transient live checkpoint remembered ctx=%s live=%d visible=%zu stripped=%zu",
               ctx, ds4_session_pos(s->session), strlen(visible), strlen(stripped));
    trace_event(s, trace_id,
                "transient live checkpoint remembered: live=%d visible=%zu stripped=%zu",
                ds4_session_pos(s->session), strlen(visible), strlen(stripped));
    free(stripped);
    free(visible);
}
...
decode_again:
    ...
    if (!parsed_calls.len &&
        ...
        remember_transient_checkpoint(s, j, ctx_span, trace_id,
                                      parsed_content ? parsed_content : "",
                                      parsed_reasoning,
                                      parsed_calls.len ? &parsed_calls : NULL);
    } else {
