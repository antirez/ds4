...
static int live_text_prefix_prompt(server *s, server_slot *slot,
    ...
    if (req->kind == REQ_COMPLETION) {
        ds4_tokens start = {0};
        ds4_chat_begin(s->engine, &start);
        /* Raw string prompts include this start sequence. A previous exact
         * token-array request may lack it, even when its visible text matches. */
        if (!ds4_tokens_starts_with(live_tokens, &start)) {
            ds4_tokens_free(&start);
            free(live_text);
            return 0;
        }
        char *start_text = render_tokens_text(s->engine, &start, &live_text_start);
        free(start_text);
        ds4_tokens_free(&start);
    }
    ...
    bool ok = build_live_prompt_suffix(
        s, slot, req, req->prompt_text + live_text_len - live_text_start,
        effective_prompt);
    free(live_text);
