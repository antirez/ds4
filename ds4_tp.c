...
/* v13 over upstream v12: upstream v12 added the mirrored-sync checkpoint
 * frame (DS4_TP_FRAME_SYNC_CHECKPOINT); this fork's OdinLink negotiation and
 * mirrored-sync frames moved past it (SYNC_GO 22, SYNC_CHECKPOINT 23,
 * ODL_READY 24). */
 ...
 * reliably.  The control socket does, so sample it while polling a
 * data-plane completion and abort before the GPU command-buffer watchdog
 * fires. */
...
int ds4_tp_gate_exchange(ds4_tp *tp, uint32_t layer, uint32_t gate, uint64_t seq) {
    ...
    if (ph.magic != DS4_TP_MAGIC || ph.layer != layer || ph.gate != gate ||
        ...
        fprintf(stderr,
                "ds4-tp: gate desync: got l=%u g=%u seq=%llu b=%llu, "
                "want l=%u g=%u seq=%llu b=%llu\n",
                ph.layer, ph.gate, (unsigned long long)ph.seq,
